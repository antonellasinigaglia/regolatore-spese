(function(){
  ...
  function money(s){
    const m=String(s||'').match(/(?:^|\s)(\d{1,3}[.,]\d{2})(?=\s*$|\s)/);
    if(!m)return null;
  ...
  function isNoise(s){
    return /^(shopper|scontrino|totale|pagamento|contanti|carta|resto|punti|fidelity|tessera|iva|subtotale|buono|coupon|sacchetto)/i.test(s)
      || /sconto|coupon|buono|pagamento|contanti|resto|totale|tessera|punti|fidelity|sacchetto compost|ce93[\s\/]?42/i.test(s);
  }

  function parseItems(result){
    const all=(result.items||[]).map(it=>({text:clean(it.text),score:Number(it.score||0),...boxOf(it.poly)})).filter(x=>x.text&&x.score>=0.30);
    if(!all.length)return [];

    const ys=all.flatMap(x=>[x.y0,x.y1]);
    const minY=Math.min(...ys), maxY=Math.max(...ys), span=maxY-minY;
    const body=all.filter(x=>x.y0>minY+span*.12 && x.y1<minY+span*.82);
  ...
  function totalFromResult(result){
    for(const it of [...(result.items||[])].reverse()){
      ...
      if(/totale\s*(euro|€)?|totale da pagare|totale complessivo/i.test(t)){
        const p=money(t); if(p!==null)return p;
      }
    }
  ...
  async function getEngine(){
    if(!enginePromise){
      enginePromise=import('https://cdn.jsdelivr.net/npm/@paddleocr/paddleocr-js@0.4.2/+esm').then(async mod=>{
        if(!mod.PaddleOCR)throw new Error('PaddleOCR.js non ha esportato PaddleOCR');
  ...
  window.scanReceipt=async function(file){
    ...
    box.innerHTML='<div class="card"><p>Carico il lettore OCR…</p><div class="muted small">La prima lettura può richiedere alcuni secondi. Tutto viene eseguito nel browser.</div></div>';
    try{
