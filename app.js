...
async function loadFromDB(){
  ...
  const [settings,categories,expenses,annual,subscriptions]=await Promise.all([
    ...
    db.from("categories").select("id,name,budget,active,due_date").eq("user_id",uid).order("name"),
  ...
  state={
    ...
    categories:(categories.data||[]).map(c=>({id:c.id,name:c.name,budget:Number(c.budget),active:c.active,dueDate:c.due_date||""})),
...
async function seedDefaults(uid){
  ...
  const categories=defaultCategories.map(([name,budget])=>({id:crypto.randomUUID(),user_id:uid,name,budget,active:true,due_date:null}));
...
function upcomingExpenses(){
 ...
 const items=state.categories.filter(c=>c.active&&c.dueDate).filter(c=>{const d=new Date(c.dueDate+"T12:00:00");return d>=start&&d<=end}).sort((a,b)=>a.dueDate.localeCompare(b.dueDate));
 if(!items.length)return `<div class="empty">Nessuna spesa imminente nei prossimi 5 giorni.</div>`;
 return items.map(c=>`<div class="row upcoming-row"><div style="flex:1"><b>${esc(c.name)}</b><div class="muted">Scadenza ${fmtDate(c.dueDate)} · ${money(c.budget)}</div></div><span class="upcoming-date">${fmtDate(c.dueDate)}</span></div>`).join("");
}
...
function budgetPage(){const active=state.categories.filter(c=>c.active),archived=state.categories.filter(c=>!c.active);const activeHtml=active.map(c=>`<div class="row"><div style="flex:1"><b>${esc(c.name)}</b><div class="muted">Attiva · speso ${money(spent(c.id))}${c.dueDate?` · scadenza ${fmtDate(c.dueDate)}`:""}</div></div><span>${money(c.budget)}</span><button class="secondary" onclick="editCategory('${c.id}')">Modifica</button></div>`).join("")||`<div class="empty">Nessuna categoria attiva.</div>`;const archivedHtml=archived.length?`<div class="section"><details><summary class="muted" style="cursor:pointer">Categorie archiviate (${archived.length})</summary><div style="margin-top:12px">${archived.map(c=>`<div class="row"><div style="flex:1"><b>${esc(c.name)}</b><div class="muted">Archiviata · speso ${money(spent(c.id))}${c.dueDate?` · scadenza ${fmtDate(c.dueDate)}`:""}</div></div><span>${money(c.budget)}</span><button class="secondary" onclick="editCategory('${c.id}')">Riattiva</button></div>`).join("")}</div></details></div>`:"";return `<div class="section"><div class="actions"><button class="primary" onclick="addCategory()">+ Categoria</button></div><div class="section">${activeHtml}</div>${archivedHtml}</div>`}
...
function addCategory(){document.body.insertAdjacentHTML("beforeend",`<div class="modal" id="modal"><div class="modalbox"><h2>Nuova categoria</h2><div class="form"><div class="field"><label>Nome</label><input id="cname"></div><div class="field"><label>Budget mensile €</label><input id="cbudget" type="number" step="0.01"></div><div class="field"><label>Scadenza</label><input id="cdue" type="date"></div><div class="actions"><button class="primary" onclick="confirmCategory()">Crea</button><button class="secondary" onclick="closeModal()">Annulla</button></div></div></div></div>`)}
async function confirmCategory(){let n=document.getElementById("cname").value.trim(),b=Number(document.getElementById("cbudget").value),due=document.getElementById("cdue").value||null;if(!n||b<0)return alert("Inserisci nome e budget.");const {error}=await db.from("categories").insert({id:crypto.randomUUID(),user_id:currentUser.id,name:n,budget:b,active:true,due_date:due});if(error)return alert(error.message);closeModal();await loadFromDB();render()}
function editCategory(id){let c=state.categories.find(x=>x.id===id);document.body.insertAdjacentHTML("beforeend",`<div class="modal" id="modal"><div class="modalbox"><h2>Modifica categoria</h2><div class="form"><div class="field"><label>Nome</label><input id="cname" value="${esc(c.name)}"></div><div class="field"><label>Budget mensile €</label><input id="cbudget" type="number" step="0.01" value="${c.budget}"></div><div class="field"><label>Scadenza</label><input id="cdue" type="date" value="${c.dueDate||""}"></div><div class="actions"><button class="primary" onclick="updateCategory('${id}')">Salva</button><button class="secondary" onclick="archiveCategory('${id}')">${c.active?"Archivia":"Riattiva"}</button><button class="secondary" onclick="closeModal()">Annulla</button></div></div></div></div>`)}
async function updateCategory(id){let c=state.categories.find(x=>x.id===id),name=document.getElementById("cname").value.trim()||c.name,budget=Number(document.getElementById("cbudget").value)||0,due=document.getElementById("cdue").value||null;const {error}=await db.from("categories").update({name,budget,due_date:due}).eq("id",id).eq("user_id",currentUser.id);if(error)return alert(error.message);closeModal();await loadFromDB();render()}
...
async function resetData(){if(!confirm("Ripristinare i dati iniziali? Le spese attuali verranno cancellate."))return;try{await replaceAllData({income:3000,savings:1000,categories:defaultCategories.map(([name,budget])=>({id:crypto.randomUUID(),name,budget,active:true,dueDate:""})),expenses:[],annual:[{id:crypto.randomUUID(),name:"Prime Video",amount:49,date:"",note:"Pagamento unico annuale"}],subscriptions:defaultSubscriptions.map(([name,amount,frequency,active])=>({id:crypto.randomUUID(),name,amount,frequency,active}))});await loadFromDB();render()}catch(e){alert(e.message)}}
